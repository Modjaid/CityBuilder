    ...
    public static class LocalizationManager
    {
        ...
        public static string Localize(string localizationKey)
        {
            ...
            if(!Dictionary[Language].ContainsKey(localizationKey)){
                Debug.LogError($"Ключ локализации не найден: {localizationKey}");
            }
            return Dictionary[Language][localizationKey];
        }

        /// <summary>
		/// first index is MainText
		/// </summary>
        public static string LocalizeWithTags(string localKey, Dictionary<string, string> tags){
            string text = Localize(localKey);

            foreach (var tag in tags)
            {
                var word = Localize(tag.Value);
                text = text.Replace(tag.Key, word);
            }
            return text;
        }
